...
export async function loadTeamData() {
  try {
    ...
    const teamsUrl = SeasonManager.getUrl("tableteams");
    ...
    const teamData = [];
    const racePointsTeams = [];

    // ✅ Adjusted: we only need valid rows with at least 15 columns
for (let i = 1; i < rows.length && i <= 7; i++) {
      ...
      raceResults.forEach((points, idx) => {
        ...
        if (!racePointsTeams[idx]) racePointsTeams[idx] = [];
        if (numPoints > 0) racePointsTeams[idx].push(numPoints);
      });
    }

    // Podium thresholds
    const raceThresholdsTeams = racePointsTeams.map(points => {
    ...
    // ✅ Dynamic header (based on how many races there are)
    const numRaces = teamData[0]?.raceResults.length || 9;
    teamThead.innerHTML = `
      <th>Rank</th>
...
function renderRow(entity, thresholds, isDriver, rankIndex) {
  ...
  let row = `
    ...
    <td class="tc-empty" data-team="${entity.team}" style="--team-color:${entity.teamColor}">
