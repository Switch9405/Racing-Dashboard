    ...
    Object.entries(eventsMap).forEach(([eventNumber, { date, races }]) => {
      ...
      races.forEach((race, idx) => {
        ...
        if (format === "MULTICLASS" || format === "FLIGHT CLUB") {
          ...
          // ✅ Adding data-race here
          const raceCell = document.createElement('td');
          raceCell.textContent = race.race || ""; 
          raceCell.setAttribute('data-race', race.race);
          raceCell.setAttribute('data-label', 'Race');
          raceCell.setAttribute('rowspan', '2');
          
          row1.appendChild(raceCell);

          row1.innerHTML += `
            <td data-label="Track" rowspan="2">${race.track || ""}</td>
            <td data-label="Layout" rowspan="2">${race.layout || ""}</td>
            <td data-label="Format" rowspan="2">${race.format || ""}</td>
            <td class="rc-classes" style="vertical-align: middle; text-align: center;">${label1 || ""}</td>
            <td data-label="Car">${race.car1 || ""}</td>
            <td class="rc-classes">${label3 || ""}</td>
            <td data-label="Pole Position">${race.pole1 || ""}</td>
            <td data-label="Fastest Lap">${race.fastdriver1 || ""}</td>
            <td data-label="Lap Time">${race.fastlap1 || ""}</td>
          `;
          ...
          row2.innerHTML += `
            ...
            <td data-label="Car">${race.car2 || ""}</td>
            <td class="rc-classes">${label4 || ""}</td>
            <td data-label="Pole Position">${race.pole2 || ""}</td>
            ${isRace10 
              ? '<td></td><td></td>'
              : `<td data-label="Fastest Lap">${race.fastdriver2 || ""}</td><td data-label="Lap Time">${race.fastlap2 || ""}</td>`}
          `;
          ...
          row.innerHTML += `
            <td data-label="Track">${race.track || ""}</td>
            <td data-label="Layout">${race.layout || ""}</td>
            <td data-label="Format">${race.format || ""}</td>
            <td></td>
            <td data-label="Car">${race.car1 || ""}</td>
            <td></td>
            <td data-label="Pole Position ">${race.pole1 || ""}</td>
        ...
        if (raceNum === 9) {
          ...
          overlayRow.innerHTML = `
            <td colspan="100%" style="position: relative; padding: 0; height: 100px;">
