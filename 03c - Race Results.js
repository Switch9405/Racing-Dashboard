...
export async function loadRaceResults() {
  try {
    // ✅ Use SeasonManager to get the active season's race results sheet
    const raceResultsUrl = SeasonManager.getUrl("tableresults");

    const resp = await fetch(raceResultsUrl);
    if (!resp.ok) throw new Error(`Failed to fetch race results: ${resp.status}`);
    ...
    document.getElementById("results-table-container").textContent = "Error loading race results.";
  }
}
