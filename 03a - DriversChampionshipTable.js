const driverschampUrl = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQCpD4P7hsdJXV-7Im8nxzWP-O8hakVp-9NHmKWRnmshWzZXxYdnKLcV6JgdFaXCv_vAVERiqrdbvPr/pub?output=csv&gid=1594244845";
...
export async function loadDriverData() {
  try {
    const response = await fetch(driverschampUrl);
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const pts = cells[5];
      const raceResults = cells.slice(6, 15);
      const fl = cells[15];
      const pp = cells[16];
...
// The same renderRow method as in your original script:
